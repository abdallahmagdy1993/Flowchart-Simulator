        ...
        if(bDragging == false) {
            ...
            if(pWind->GetButtonState(LEFT_BUTTON, P.x, P.y) == BUTTON_UP) {
                ...
                if(P.x != iXOld) {
					...
					if(Top.x + (P.x - iXOld)-width/2>UI.MnItWdth&&Top.x + (P.x - iXOld)+width/2<UI.width)
						
					{Top.x = Top.x + (P.x - iXOld);
					iXOld = P.x;}
                }
                if(P.y != iYOld) {
					if(Top.y + (P.y - iYOld)>UI.TlBrWdth&&Top.y + (P.y - iYOld)+height<UI.height-UI.StBrWdth)
						
					{Top.y = Top.y + (P.y - iYOld);
					iYOld = P.y;}
                }
            }
